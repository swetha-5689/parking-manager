...
class EditLayout extends React.Component {
  ...
  componentDidMount() {
    axios
      ...
      .then((response) => {
        this.setState({ floors: response.data.floorNumber });
      })
  ...
  getData(floorNum) {
    axios
      .get("http://localhost:5000/api/spots/" + floorNum)

      .then((response) => {
        this.setState({ data: response.data.floorNumber });
      })
    ...
    return this.state.floors.map((response, i) => {
      return (
        <>
          ...
          <Col>
            <ParkingSpot key={i} floorNumber={this.state.floors[i].number} />{" "}
          </Col>
        </>
      );
    });
